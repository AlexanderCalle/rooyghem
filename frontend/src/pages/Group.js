  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_HOST}/groups/` +
          params.group_name +
          "/info"
      );
      ...
      <main class="container" id="groupcontainer">
        <div class="info">
          <GroupOverview
            ...
            location_adress={locationInfo.adress}
                  ...
                  <>
                    ...
                    <div class="info">
                      {groupedAlbum.albums.map((album) => (
                        <a
                          href={`/albums/groups/${params.group_name}/${album.album_id}`}
                        >
                          <div class="interfaceinfo">
                            <div class="interfaceinfo-inner">
                              <img
                                src={`${process.env.REACT_APP_BACKEND_HOST}/public/images/album1.svg`}
                                width="50"
                                height="auto"
                              />
                              <p>{album.name}</p>
          ...
          <div id="groupleaders-content">
            {leaderInfo.map((leader) => (
              <LeaderOverview
                ...
                isBanleader={leader.is_banleader}
              />
            ))}
          </div>
        </div>
